pub mod skin_2d {
    use mcml_skin::{SkinType, skin_type_checker::skin_type_checker};
    use skia_safe::{Bitmap, Color, ImageInfo};

    use crate::skin_draw::{
        ColorMix, SCALE_TYPEB, SCALE_TYPEC, draw_mix, draw_with_fill_image, draw_with_fill_image_mix, fill_image, scale
    };
    ...
    pub fn skin_2d_draw_typeb(image: &mut Bitmap, skin_type: Option<SkinType>) -> Option<Bitmap> {
        ...
        draw_with_fill_image(&mut image1, image, 4 + 8 * 4, 4 + 8 * 8, 20, 20, 8, 12, 8, 8)?;

        // right hand
        if skintype == SkinType::NewSlim {
            draw_with_fill_image(&mut image1, image, 4 + 1 * 8, 4 + 8 * 8, 44, 20, 3, 12, 8, 8)?;
        } else {
        ...
        // left hand
        if skintype == SkinType::NewSlim {
            draw_with_fill_image(&mut image1, image, 4 + 12 * 8, 4 + 8 * 8, 36, 52, 3, 12, 8, 8)?;
        } else {
            if skintype == SkinType::Old {
                // 旧版镜像：从源图右手区域镜像到左手位置
                let img_ptr = image.pixels() as *const u8;
                ...
                for i in (0..4).rev() {
                    for j in 0..12 {
                        ...
                        fill_image(&mut image1, 4 + 12 * 8 + i * 8, 4 + 8 * 8 + j * 8, 8, 8, pix)?;
                    }
                }
            } else {
                draw_with_fill_image(&mut image1, image, 4 + 12 * 8, 4 + 8 * 8, 36, 52, 4, 12, 8, 8)?;
            }
        }

        // right leg
        draw_with_fill_image(&mut image1, image, 4 + 4 * 8, 4 + 20 * 8, 4, 20, 4, 12, 8, 8)?;
        ...
        if skintype == SkinType::Old {
            ...
            for i in (0..4).rev() {
                for j in 0..12 {
                    ...
                    fill_image(&mut image1, 4 + 8 * 8 + i * 8, 4 + 20 * 8 + j * 8, 8, 8, pix)?;
                }
            }
        } else {
            draw_with_fill_image(&mut image1, image, 4 + 8 * 8, 4 + 20 * 8, 20, 52, 4, 12, 8, 8)?;
        }
        ...
        if skintype == SkinType::NewSlim {
            // top: 右手 overlay
            draw_with_fill_image_mix(&mut image1, image, 1 * 8 + 1, 8 * 8 + 2, 44, 36, 3, 12, 9, 9)?;
            // top: 左手 overlay
            draw_with_fill_image_mix(&mut image1, image, 12 * 8 + 4, 8 * 8 + 2, 52, 52, 3, 12, 9, 9)?;
        } else if skintype == SkinType::New {
            // top: 右手 overlay
            draw_with_fill_image_mix(&mut image1, image, 0, 8 * 8 + 2, 44, 36, 4, 12, 9, 9)?;
            // top: 左手 overlay
            draw_with_fill_image_mix(&mut image1, image, 12 * 8 + 4, 8 * 8 + 2, 52, 52, 4, 12, 9, 9)?;
        }

        if skintype == SkinType::New || skintype == SkinType::NewSlim {
            // top: 右腿 overlay
            draw_with_fill_image_mix(&mut image1, image, 4 * 8 + 2, 20 * 8 - 2, 4, 36, 4, 12, 9, 9)?;
            // top: 左腿 overlay
            draw_with_fill_image_mix(&mut image1, image, 8 * 8 + 2, 20 * 8 - 2, 4, 52, 4, 12, 9, 9)?;
        }
