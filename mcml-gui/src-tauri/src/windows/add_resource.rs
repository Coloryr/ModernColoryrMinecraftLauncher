...
fn build_status(app: &AppHandle) -> ResourceStatusDto {
    ...
    ResourceStatusDto {
        window_open: app.get_webview_window("mcml-add_resource").is_some(),
        tasks: map
            .values()
            .flat_map(|entry| {
...
pub async fn add_resource_download(
    ...
    let dir = {
        ...
        match file_type {
            ...
            _ => mcml_downloader::get_download_path(),
        }
    };
    ...
    tauri::async_runtime::spawn(async move {
        let ok = mcml_downloader::start_download_task(vec![obj]).await;
