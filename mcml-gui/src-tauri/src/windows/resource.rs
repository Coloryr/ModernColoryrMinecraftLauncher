...
//!
//! 列表复用 mcml-game 的扫描（模组元数据、材质包 pack.mcmeta、存档 level.dat、
...
fn parse_instance(uuid: &str) -> Result<GameInstance, String> {
    ...
    mcml_game::get_instance(&uuid).ok_or_else(|| "err.gameNotFound".to_string())
}
...
async fn load_saves(instance: GameInstance) -> Result<Vec<mcml_game::game_saves::SaveObj>, String> {
