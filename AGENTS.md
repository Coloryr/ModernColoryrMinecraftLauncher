...
- **例外：只小幅度改前端时不需要关闭。** 涉及 `mml-vue/` 下少量文件的样式 / 文案 /
  组件微调（不碰 Rust 侧、不加窗口 / 命令）可以直接改，vite 热更新自己会把改动送上去；
  改完仍要跑 `npm run build` 验证。超出这个范围（改 `.rs`、动窗口结构、批量改前端）
  仍按上一条先关闭。
