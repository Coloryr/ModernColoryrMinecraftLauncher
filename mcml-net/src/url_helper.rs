...
use crate::net::{authlib_api::{ArtifactsObj, AuthlibInjectorObj}, optifine_api::OptifineListObj};
...
/// 获取minecraft资源下载地址
pub fn get_download_assets(hash: &String) -> String {
...
/// - `version`: 游戏版本
pub fn get_minecraft_client(version: &String) -> String {
...
/// 获取forge版本信息获取网址
/// - `version`: 游戏版本
pub fn get_forge_versions(version: &String) -> String {
    match get_source() {
        SourceLocal::Offical => {
...
/// 获取neoforge信息获取网址
pub fn get_neoforge_meta(version: &String) -> String {
    match get_source() {
        SourceLocal::Offical => {
            format!("{}api/maven/versions/releases/net%2Fneoforged%2Fneoforge", urls::NEOFORGE)
        }
...
/// - `mc`: 游戏版本
/// - `version`: forge版本
pub fn get_forge_jar(mc: &String, version: &String) -> String {
    match get_source() {
...
/// - `mc`: 游戏版本
/// - `version`: forge版本
pub fn get_neoforge_jar(v2222: bool, mc: &String, version: &String) -> String {
    let url = if v2222 {
...
pub fn get_optifine_jar(obj: &OptifineListObj) -> String {
    format!("{}optifine/{}/{}/{}", urls::BMCLAPI, obj.mcversion, obj.rtype, obj.patch)
}

/// 替换运行库下载地址
/// - `url`: 运行库地址
pub fn replace_minecraft_libraries(url: &String) -> String {
    match get_source() {
        SourceLocal::Offical => url.clone(),
...
/// 替换运行库下载地址
/// - `url`: 运行库地址
pub fn replace_forge_libraries(url: &String) -> String {
    match get_source() {
        SourceLocal::Offical => url.clone(),
...
/// 替换运行库下载地址
/// - `url`: 运行库地址
pub fn replace_neoforge_libraries(url: &String) -> String {
    match get_source() {
        SourceLocal::Offical => url.clone(),
        SourceLocal::Bmclapi => url.replace(urls::FORGE, &format!("{}maven/", urls::BMCLAPI)),
    }
...
/// 替换fabric库下载地址
pub fn replace_fabric_libraries(url: &String) -> String {
    match get_source() {
        SourceLocal::Offical => url.clone(),
...
/// 修正Forge下载地址
pub fn forge_url_fix(url: String) -> String {
    if url.eq_ignore_ascii_case("1.7.2") {
        String::from("-mc172")
    } else if url.eq_ignore_ascii_case("1.7.10") {
        String::from("-1.7.10")
    } else if url.eq_ignore_ascii_case("1.8.9") {
        String::from("-1.8.9")
    } else if url.eq_ignore_ascii_case("1.9") {
        String::from("-1.9.0")
    } else if url.eq_ignore_ascii_case("1.9.4") {
        String::from("-1.9.4")
    } else if url.eq_ignore_ascii_case("1.10") {
        String::from("-1.10.0")
    } else {
        String::new()
    }
