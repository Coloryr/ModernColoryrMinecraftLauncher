...
mod tests {
    ...
    fn test_save_and_open_bitmap_round_trip() {
        ...
        let path =
            std::env::temp_dir().join(format!("mcml_skin_roundtrip_{}.png", std::process::id()));
        save_bitmap(&bm, &path);
        ...
        let missing =
            std::env::temp_dir().join(format!("mcml_skin_missing_{}.png", std::process::id()));
    ...
    fn test_open_bitmap_invalid_file() {
        let path =
            std::env::temp_dir().join(format!("mcml_skin_invalid_{}.txt", std::process::id()));
